...
function App() {
  ...
  const getCurrentUser = () => {
    return mainApi.getUser().then((data) => {
      setCurrentUser({
        ...currentUser,
        ...data.data,
      });
  ...
  const updateCurrentUser = (user) => {
    return mainApi.updateUser(user).then((data) => {
      setCurrentUser({
        ...currentUser,
        ...data.data,
      });
  ...
  const handleRegister = (values) => {
    return mainApi.signup(values).then((user) => {
      return handleLogin({
        email: values.email,
  ...
  const handleSaveMovie = (movie) => {
    return mainApi.saveMovie(movie).then((data) => {
      setSavedMovies([...savedMovies, data.data]);
    });
  };

  const handleUnSaveMovie = (movieId) => {
    return mainApi.unSaveMovie(movieId).then((data) => {
      setSavedMovies(
        savedMovies.filter((movie) => movie._id !== data.data._id)
      );
    });
  };
